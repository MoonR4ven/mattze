...
export class BillbeeAPI {
  ...
  async getCustomers(options?: {
    page?: number
    pageSize?: number
  }): Promise<{ success: boolean; customers?: any[]; totalCount?: number; error?: string }> {
    try {
      const params = new URLSearchParams()
      if (options?.page) params.append("page", options.page.toString())
      if (options?.pageSize) params.append("pageSize", options.pageSize.toString())

      const url = `${this.baseUrl}/customers?${params.toString()}`
      ...
      console.error("Error listing Billbee customers:", error)
      return {
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
      }
  ...
  async createCustomer(customerData: {
    ...
    try {
      const body = {
        ...
        CountryCode: customerData.countryCode || "NL",
      }
      ...
      console.error("Error creating Billbee customer:", error)
      return {
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
      }
  ...
  async getOrderStates(): Promise<{ success: boolean; states?: any[]; error?: string }> {
    try {
      const response = await fetch(`${this.baseUrl}/enums/orderstates`, {
        method: "GET",
        headers: this.getAuthHeaders(),
      })
      ...
      console.error("Error fetching Billbee order states:", error)
      return {
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
      }
  ...
  async getPaymentTypes(): Promise<{ success: boolean; paymentTypes?: any[]; error?: string }> {
    try {
      const response = await fetch(`${this.baseUrl}/enums/paymenttypes`, {
        method: "GET",
        headers: this.getAuthHeaders(),
      })
      ...
      console.error("Error fetching Billbee payment types:", error)
      return {
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
      }
  ...
  async updateCustomer(customerData: {
    ...
    try {
      ...
      if (existingCustomer?.Id) {
        ...
        return {
          success: true,
          message: "New customer - email will be included in order data",
        }
      }
